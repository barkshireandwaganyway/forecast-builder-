...
function bindEvents(){
  ...
  ['locationLabel','slideTitle','slideSubtitle'].forEach(id => $(id).addEventListener('input', renderAll));
  $('dayCount').addEventListener('input', () => {
    const title = $('slideTitle');
    if (!title.value.trim() || /^\s*\d+\s+DAY\s+FORECAST\s*$/i.test(title.value)) {
      title.value = `${$('dayCount').value} DAY FORECAST`;
    }
    renderAll();
  });
...
async function exportPng(){
  ...
  try {
    ...
    setStatus('PNG exported from the locked 1920×1080 slide canvas. Preview and export use the same alignment rules.');
  } catch (err) {
    console.error(err);
...
function handleProductMapUpload(e){
  const file = e.target.files && e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = () => {
    const p = ensureProduct();
    p.mapImageUrl = reader.result;
    renderProduct();
    renderProductEditor();
    setStatus('Custom map image loaded into the selected product slide.');
  };
  reader.readAsDataURL(file);
}
...
function renderProductEditor(){
  ...
  $('productMapUpload')?.addEventListener('change', handleProductMapUpload);
}


/* ----------------------- V12 product slide repair -----------------------
   Goal: product slides use a stable broadcast layout, do not reuse the day-card grid,
   keep Rain Chances + Air Quality panel style, and give every map/key/source overlay
   a real on/off + X/Y control. No image generation. Code only. */
function defaultProductState(){
  return {
    type:'forecast', dayOffset:0, title:'', subtitle:'', kicker:'', summary:'', sourceNote:'',
    data:null, updated:'', manualMapValues:'',
    showFront:false, frontType:'cold', frontX:310, frontY:240, frontAngle:12, frontLength:700,
    showMapKey:true, mapKeyX:36, mapKeyY:36,
    showCityLabels:true, showSourceBar:true, sourceBarX:36, sourceBarY:786, sourceBarWidth:1660,
    showHazardPanel:true, hazardPanelX:1035, hazardPanelY:72,
    showProductData:true, mapImageUrl:'', mapImageOpacity:85, dataFieldOpacity:72
  };
}
function ensureProduct(){
  state.product = Object.assign(defaultProductState(), state.product || {});
...
function productLayoutClass(type){
  if (type === 'storm-hazards') return 'splitProduct fixedProduct';
  if (type === 'rain-chances' || type === 'air-quality') return 'panelOnly fixedProduct';
  if (['temperatures','dewpoints','heat-index','clouds-radar','severe-outlook'].includes(type)) return 'fullMap fixedProduct';
  return '';
}
function productTypeToMapClass(type){
  if (type === 'temperatures') return 'tempMap';
  ...
  if (type === 'heat-index') return 'heatMap';
  if (type === 'clouds-radar') return 'radarMap';
  if (type === 'severe-outlook' || type === 'storm-hazards') return 'severeMap';
  return 'generalMap';
}
function renderProduct(){
  const p = ensureProduct();
  ...
  $('issueTime').textContent = p.updated ? `UPDATED ${p.updated}` : (state.modules.issueTime || 'READY');
  const typeClass = `product-${String(p.type).replace(/[^a-z0-9-]/gi,'')}`;
  productCanvas.className = `productCanvas ${productLayoutClass(p.type)} ${typeClass}`.trim();
  productCanvas.innerHTML = productHtml(p);
}
function productHtml(p){
  const baseData = p.data || buildProductDataFromCurrent(p.type);
...
function inlineProductStyle(p){
  return `--key-x:${Number(p.mapKeyX||0)}px;--key-y:${Number(p.mapKeyY||0)}px;--source-x:${Number(p.sourceBarX||0)}px;--source-y:${Number(p.sourceBarY||0)}px;--source-w:${Number(p.sourceBarWidth||1660)}px;--field-opacity:${Math.max(0,Math.min(1,Number(p.dataFieldOpacity ?? 72)/100))};--img-opacity:${Math.max(0,Math.min(1,Number(p.mapImageOpacity ?? 85)/100))};`;
}
function productSourceNote(data,p,fallback=''){
  return p.sourceNote || data.sourceNote || fallback || 'Source: NWS point forecast, NWS grid data, and editable RBRTW product logic.';
}
function productSummaryText(data,p,fallback=''){
  return p.summary || data.summary || fallback || '';
}
function mapShellHtml(data, p, mapClass, innerHtml, options={}){
  const customUrl = String(p.mapImageUrl || '').trim();
  const customImg = customUrl ? `<img class="mapBaseImage customMapImage" src="${escapeAttr(proxyMaybe(customUrl))}" alt="custom weather map image">` : '';
  const base = customImg || (data.basemapImage ? `<img class="mapBaseImage" src="${escapeAttr(data.basemapImage)}" alt="regional map base">` : `<div class="mapBaseFallback"><div class="fallbackStateShape"></div><div class="fallbackCoast"></div></div>`);
  const overlaySrc = data.overlayImage || data.outlookImage || data.radarImage || '';
  const overlay = overlaySrc ? `<img class="noaaOverlayImage" src="${escapeAttr(overlaySrc)}" alt="NOAA/NWS weather map overlay">` : '';
  const field = options.fieldClass && p.showProductData !== false ? `<div class="dataField ${options.fieldClass}"></div>` : '';
  const legend = (p.showMapKey !== false && options.legend) ? `<div class="floatingKey">${options.legend}</div>` : '';
  const source = p.showSourceBar !== false ? `<div class="mapProductFooter">${escapeHtml(productSummaryText(data,p,options.defaultSummary||''))}${productSummaryText(data,p,options.defaultSummary||'') ? '<br>' : ''}${escapeHtml(productSourceNote(data,p,options.defaultNote||''))}</div>` : '';
  const front = p.showFront ? `<div class="frontLine" style="left:${Number(p.frontX||0)}px;top:${Number(p.frontY||0)}px;width:${Number(p.frontLength||0)}px;transform:rotate(${Number(p.frontAngle||0)}deg)"></div>` : '';
  const status = '';
  return `<div class="productMap ${mapClass}" style="${inlineProductStyle(p)}">${base}${field}${overlay}<div class="mapTexture"></div><div class="countyGrid"></div><div class="roadGrid"></div>${front}${legend}${innerHtml}${status}${source}</div>`;
}
function proxyMaybe(url){
  try {
    const u = new URL(url, location.href);
    if (u.origin === location.origin) return u.toString();
    if (location.protocol.startsWith('http')) return proxyUrl(u.toString());
    return u.toString();
  } catch { return url; }
}
function cityLabelsForMap(points, key='temp', suffix='°'){
  const pts = Array.isArray(points) && points.length ? points : PRODUCT_CITIES.map(c => ({...c, [key]:''}));
  return pts.map(pt => {
    const val = cleanDisplay(pt[key]);
    const small = ['Dallas','Abilene','Brownsville','Corpus Christi','Houston','Del Rio','Laredo'].includes(pt.name) ? ' small' : '';
    const valueHtml = val ? `<span class="tempValue">${escapeHtml(val)}${escapeHtml(suffix)}</span>` : '';
...
function pointMapHtml(data, p, mapClass, valueKey, suffix){
  const points = data.points || pointsFromSelectedDay(valueKey);
  const fieldClass = valueKey === 'temp' ? 'tempField' : valueKey === 'dewpoint' ? 'dewField' : 'heatField';
  const labels = p.showCityLabels !== false ? cityLabelsForMap(points, valueKey, suffix) : '';
  return mapShellHtml(data, p, mapClass, labels, {
    legend: mapLegendForProduct(valueKey), fieldClass,
    defaultSummary: defaultMapSummary(valueKey, points),
    defaultNote: valueKey === 'temp' ? 'Source: NWS/Open-Meteo forecast values with editable regional map layer.' : 'Source: NWS/Open-Meteo forecast values with editable regional map layer.'
  });
}
function defaultMapSummary(valueKey, points){
  const vals = (points || []).map(x => Number(x[valueKey])).filter(Number.isFinite);
  if (!vals.length) return '';
  const max = Math.max(...vals), min = Math.min(...vals);
  if (valueKey === 'dewpoint') return `Selected day dew points range near ${min}° to ${max}°.`;
  if (valueKey === 'heatIndex') return `Selected day heat index values peak near ${max}°.`;
  return `Selected day temperatures range near ${min}° to ${max}°.`;
}
function mapLegendForProduct(valueKey){
  if (valueKey === 'dewpoint') return `<div class="mapLegendBox dewLegend"><div class="legendRow legendHigh">70 - 80+<br>Oppressive</div><div class="legendRow legendMid">60 - 69<br>Humid</div><div class="legendRow legendDry">50 - 59<br>Dry</div><div class="legendRow legendVeryDry">40 - 49<br>Very Dry</div><div class="legendRow legendLow">20 - 39<br>Bone Dry</div></div>`;
  if (valueKey === 'heatIndex') return `<div class="mapLegendBox heatLegend"><div class="legendRow legendExtreme">115°+<br>Extreme</div><div class="legendRow legendHot">105 - 114°<br>Dangerous</div><div class="legendRow legendMid">95 - 104°<br>Hot</div><div class="legendRow legendLow">Below 95°<br>Lower Risk</div></div>`;
  return `<div class="mapLegendBox tempLegend"><div class="legendRow legendExtreme">100°+<br>Very Hot</div><div class="legendRow legendHot">90 - 99°<br>Hot</div><div class="legendRow legendMid">80 - 89°<br>Warm</div><div class="legendRow legendLow">Below 80°<br>Cooler</div></div>`;
}
function stormHazardsHtml(data, p){
  const hazards = data.hazards || defaultHazardsFromDays();
  const labels = p.showCityLabels !== false ? cityLabelsForMap(data.points || PRODUCT_CITIES.map(c => ({...c,temp:''})), 'temp', '') : '';
  const riskKey = p.showMapKey !== false ? `<div class="floatingKey"><div class="riskLegend stormRiskLegend"><div class="non">NON SEVERE</div><div class="marg">MARGINAL</div><div class="slgt">SLIGHT</div><div class="enh">ENHANCED</div><div class="mod">MODERATE</div><div class="high">HIGH</div></div></div>` : '';
  const map = `<div class="productMap stormHazardMap severeMap" style="${inlineProductStyle(p)}">${data.basemapImage?`<img class="mapBaseImage" src="${escapeAttr(data.basemapImage)}" alt="regional map base">`:`<div class="mapBaseFallback"><div class="fallbackStateShape"></div></div>`}<div class="dataField severeField"></div><div class="mapTexture"></div><div class="countyGrid"></div><div class="roadGrid"></div>${riskKey}${labels}<div class="mapBadge">${escapeHtml(data.overallRisk || 'MARGINAL')}</div>${p.showSourceBar!==false?`<div class="mapProductFooter">${escapeHtml(productSummaryText(data,p,`Selected day: ${dayName(selectedProductDate())}. Overall local hazard signal: ${data.overallRisk || 'MARGINAL'}.`))}<br>${escapeHtml(productSourceNote(data,p,'Source: NWS active alerts and selected-day forecast wording.'))}</div>`:''}</div>`;
  const panel = p.showHazardPanel !== false ? `<div class="productPanel hazardIndexPanel" style="left:${Number(p.hazardPanelX||1035)}px;top:${Number(p.hazardPanelY||72)}px"><div class="riskScale"><span>LOW</span><span>MODERATE</span><span>HIGH</span></div><div class="hazardBars">${hazards.map(h => hazardBarHtml(h)).join('')}</div></div>` : '';
  return `<div class="stormHazardsStage">${map}${panel}</div>`;
}
function severeOutlookHtml(data,p){
  const risk = data.overallRisk || 'MARGINAL';
  const num = riskNumber(risk);
  const points = data.points || PRODUCT_CITIES.map(c => ({...c,temp:''}));
  const labels = p.showCityLabels !== false ? cityLabelsForMap(points.map(x=>({...x,temp:''})), 'temp', '') : '';
  const legend = `<div class="riskLegend spcTopLegend"><div class="marg">Marginal<br>1</div><div class="slgt">Slight<br>2</div><div class="enh">Enhanced<br>3</div><div class="mod">Moderate<br>4</div><div class="high">High<br>5</div></div>`;
  const badge = `<div class="mapBadge bigRiskBadge" style="left:300px;top:330px">${num || ''}</div>`;
  return mapShellHtml(data,p,'severeMap', `${labels}${badge}`, {legend, fieldClass:'severeField', defaultSummary:'SPC/NWS severe outlook layer with editable local risk marker.', defaultNote:'Source: NOAA/NWS SPC outlook service when available; fallback layer is editable.'});
}
function cloudsRadarHtml(data,p){
  const points = data.points || pointsFromSelectedDay('temp');
  const labels = p.showCityLabels !== false ? cityLabelsForMap(points, 'temp', '°') : '';
  const blobs = (data.radarImage || data.overlayImage || p.mapImageUrl) ? '' : `<div class="cloudPatch" style="left:8%;top:12%;width:430px;height:180px"></div><div class="cloudPatch" style="left:55%;top:18%;width:520px;height:210px"></div><div class="radarBlob radarGreen" style="left:18%;top:60%;width:380px;height:160px"></div><div class="radarBlob radarYellow" style="left:25%;top:63%;width:180px;height:92px"></div><div class="radarBlob radarRed" style="left:31%;top:65%;width:80px;height:54px"></div>`;
  const legend = `<div class="radarLegend"><span>LIGHT</span><div></div><span>HEAVY</span></div>`;
  return mapShellHtml(data,p,'radarMap', `${blobs}${labels}`, {legend, fieldClass:'radarField', defaultSummary:'Latest clouds and radar snapshot for the selected area.', defaultNote:'Source: NOAA MRMS radar layer when available; fallback layer is editable.'});
}
function rainChancesHtml(data,p){
  const rawDays = (data.rainSeries && data.rainSeries.length ? data.rainSeries : (data.rainDays || shownDays()).map(d => ({date:d.date, pop:numberOnly(d.rainChance) ?? 0}))).slice(0,10);
  const days = rawDays.length ? rawDays : shownDays().map(d=>({date:d.date,pop:numberOnly(d.rainChance)||0}));
  return `<div class="productPanelSlide rainAheadSlide" style="${inlineProductStyle(p)}"><div class="rainChart"><div class="rainBands"><div>LIKELY</div><div>WIDESPREAD</div><div>SCATTERED</div><div>SCATTERED -<br>ISOLATED</div><div>SLIGHT -<br>NONE</div></div><div class="rainBars">${days.map(d => `<div class="rainColumn" style="height:${Math.max(10,Math.min(100,Number(d.pop)||0))}%"><b>${Math.round(Number(d.pop)||0)}%</b><span>${dayName(d.date)}<br>${dateShort(d.date)}</span></div>`).join('')}</div><div class="rainScale"><div>80-100%</div><div>60-80%</div><div>40-60%</div><div>20-40%</div><div>0-20%</div></div></div>${p.showSourceBar!==false?`<div class="mapProductFooter">${escapeHtml(productSourceNote(data,p,'Source: NWS/Open-Meteo probability of precipitation for selected forecast window.'))}</div>`:''}</div>`;
}
function airQualityHtml(data,p){
  const aqi = Number(data.aqi || 55);
  const cat = data.category || aqiCategory(aqi);
  const point = aqi <= 50 ? 0 : aqi <= 100 ? 1 : aqi <= 150 ? 2 : aqi <= 200 ? 3 : aqi <= 300 ? 4 : 5;
  return `<div class="productPanelSlide airQualitySlide" style="${inlineProductStyle(p)}"><div class="aqiScale"><div class="aqiGood">Good</div><div class="aqiModerate">Moderate</div><div class="aqiUsG">Unhealthy for<br>Sensitive Groups</div><div class="aqiUnhealthy">Unhealthy</div><div class="aqiVery">Very Unhealthy</div><div class="aqiHaz">Hazardous</div></div><div class="aqiBox"><div class="productKicker">AIR QUALITY INDEX</div><div class="aqiTicker"><span style="grid-column:${Math.max(1,Math.min(6,point+1))}"></span></div><div class="productBigNumber">${escapeHtml(aqi)}</div><div class="productCategory">${escapeHtml(cat)}</div><div class="productSummary">${escapeHtml(productSummaryText(data,p,'Selected-day air quality forecast graphic.'))}</div></div>${p.showSourceBar!==false?`<div class="mapProductFooter">${escapeHtml(productSourceNote(data,p,'Source: Open-Meteo Air Quality API, U.S. AQI forecast variable.'))}</div>`:''}</div>`;
}
function handleProductField(e){
  const p = ensureProduct();
  const key = e.target.dataset.productField;
  let val = e.target.value;
  const numericKeys = ['frontX','frontY','frontAngle','frontLength','mapKeyX','mapKeyY','sourceBarX','sourceBarY','sourceBarWidth','hazardPanelX','hazardPanelY','mapImageOpacity','dataFieldOpacity'];
...
function rangeControl(label,key,min,max,step=1){
  const p = ensureProduct();
  const val = p[key] ?? 0;
  return `<label>${label}<input data-product-field="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${escapeAttr(val)}"><input data-product-field="${key}" type="number" value="${escapeAttr(val)}"></label>`;
}
function renderProductEditor(){
  const box = $('productEditor');
  if (!box) return;
  ...
  const spec = PRODUCT_TYPES[p.type] || PRODUCT_TYPES.forecast;
  if (p.type === 'forecast') { box.innerHTML = 'Forecast Day Builder mode is active. Use the Day tab to edit each forecast card.'; return; }
  const isMap = ['storm-hazards','temperatures','dewpoints','heat-index','clouds-radar','severe-outlook'].includes(p.type);
  box.innerHTML = `<div class="productEditorGrid"><button id="refetchProductData" class="primaryBtn fullBtn">Re-fetch Map + Product Data</button><label>Product title<input data-product-field="title" value="${escapeAttr(p.title || spec.title)}"></label><label>Product subtitle<input data-product-field="subtitle" value="${escapeAttr(p.subtitle || spec.subtitle)}"></label><label>Summary / lower note<textarea data-product-field="summary">${escapeHtml(p.summary || p.data?.summary || '')}</textarea></label><label>Source note<textarea data-product-field="sourceNote">${escapeHtml(p.sourceNote || p.data?.sourceNote || '')}</textarea></label>${isMap?`<div class="sectionTitle smallTitle">Map / Image</div><label>Optional custom map image URL<input data-product-field="mapImageUrl" placeholder="https://...png/jpg" value="${escapeAttr(p.mapImageUrl || '')}"></label><label>Upload custom map image<input id="productMapUpload" type="file" accept="image/*"></label><label class="checkRow"><input type="checkbox" data-product-bool="showProductData" ${p.showProductData !== false ? 'checked':''}> Show generated color field</label>${rangeControl('Map image opacity','mapImageOpacity',0,100)}${rangeControl('Generated data field opacity','dataFieldOpacity',0,100)}<label class="checkRow"><input type="checkbox" data-product-bool="showCityLabels" ${p.showCityLabels !== false ? 'checked':''}> Show city labels / values</label><div class="sectionTitle smallTitle">Map Key / Legend</div><label class="checkRow"><input type="checkbox" data-product-bool="showMapKey" ${p.showMapKey !== false ? 'checked':''}> Show map key</label>${rangeControl('Key X','mapKeyX',0,1500)}${rangeControl('Key Y','mapKeyY',0,760)}<div class="sectionTitle smallTitle">Pressure / Front Overlay</div><label class="checkRow"><input type="checkbox" data-product-bool="showFront" ${p.showFront === true ? 'checked':''}> Show pressure/front overlay</label>${rangeControl('Front X','frontX',0,1600)}${rangeControl('Front Y','frontY',0,780)}${rangeControl('Front angle','frontAngle',-45,45)}${rangeControl('Front length','frontLength',50,1300)}`:''}${p.type==='storm-hazards'?`<div class="sectionTitle smallTitle">Storm Hazards Panel</div><label class="checkRow"><input type="checkbox" data-product-bool="showHazardPanel" ${p.showHazardPanel !== false ? 'checked':''}> Show hazard bars panel</label>${rangeControl('Hazard panel X','hazardPanelX',760,1460)}${rangeControl('Hazard panel Y','hazardPanelY',0,520)}`:''}<div class="sectionTitle smallTitle">Source Bar</div><label class="checkRow"><input type="checkbox" data-product-bool="showSourceBar" ${p.showSourceBar !== false ? 'checked':''}> Show source / note bar</label>${rangeControl('Source bar X','sourceBarX',0,1500)}${rangeControl('Source bar Y','sourceBarY',0,800)}${rangeControl('Source bar width','sourceBarWidth',320,1740)}<div class="sectionTitle smallTitle">Manual Data Override</div><label>Manual map values / notes<textarea data-product-field="manualMapValues" placeholder="Optional. Example: San Antonio=97\nAustin=95\nCorpus Christi=88">${escapeHtml(p.manualMapValues || '')}</textarea></label><button id="refreshProductRender" class="secondaryBtn">Refresh Product Layout</button></div>`;
  $$('[data-product-field]').forEach(el => el.addEventListener('input', handleProductField));
  $$('[data-product-bool]').forEach(el => el.addEventListener('change', handleProductBool));
  $('refreshProductRender')?.addEventListener('click', () => { renderProduct(); setStatus('Product layout refreshed.'); });
  $('refetchProductData')?.addEventListener('click', () => autoFillProduct());
...
async function fetchBasemapImage(){
  const urls = [
    arcgisExportUrl('https://services.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer', {format:'png32', transparent:false}),
    arcgisExportUrl('https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer', {format:'jpg', transparent:false})
  ];
  for (const url of urls) {
    const img = await imageUrlToDataUrl(url).catch(() => null);
    if (img) return {basemapImage: img, imageStatus:'Regional map base loaded'};
  }
  return {imageStatus:'Regional map base unavailable; editable fallback map shown'};
}
async function fetchNoaaRadarImage(){
  const urls = [
    arcgisExportUrl('https://mapservices.weather.noaa.gov/eventdriven/rest/services/radar/radar_base_reflectivity/MapServer', {layers:'show:0', transparent:true, imageSR:'4326'}),
    arcgisExportUrl('https://mapservices.weather.noaa.gov/eventdriven/rest/services/radar/radar_base_reflectivity/MapServer', {transparent:true, imageSR:'4326'})
  ];
  for (const url of urls) {
    const img = await imageUrlToDataUrl(url).catch(() => null);
    if (img) return {radarImage:img, overlayImage:img, imageStatus:'NOAA MRMS radar map loaded', sourceNote:'Source: NOAA/NWS MRMS Radar Base Reflectivity Map Service.'};
  }
  return {sourceNote:'NOAA MRMS image fetch unavailable. Editable radar/cloud fallback shown.'};
}
async function fetchNoaaSpcImage(dayOffset=0){
  const layers = [spcLayerForDayOffset(dayOffset), 1, 9, 17];
  for (const layer of layers) {
    const url = arcgisExportUrl('https://mapservices.weather.noaa.gov/vector/rest/services/outlooks/SPC_wx_outlks/MapServer', {layers:`show:${layer}`, transparent:true, imageSR:'4326'});
    const img = await imageUrlToDataUrl(url).catch(() => null);
    if (img) return {outlookImage:img, overlayImage:img, imageStatus:'NOAA SPC outlook map loaded', sourceNote:`Source: NOAA/NWS SPC Weather Outlooks Map Service, layer ${layer}.`};
  }
  return {sourceNote:'NOAA SPC image fetch unavailable. Editable severe outlook fallback shown.'};
}
async function autoFillProduct(){
  const p = ensureProduct();
  if (p.type === 'forecast') { setStatus('Forecast mode uses Auto Fill NWS for the day cards.'); return; }
  ...
  try {
    let data = buildProductDataFromCurrent(p.type);
    const mapNeeded = ['storm-hazards','temperatures','dewpoints','heat-index','clouds-radar','severe-outlook'].includes(p.type);
    if (mapNeeded) data = Object.assign(data, await fetchBasemapImage().catch(() => ({})));
    if (['temperatures','dewpoints','heat-index','rain-chances','air-quality'].includes(p.type)) {
      const ext = await fetchOpenMeteoProduct(p.type, selectedProductDate()).catch(err => ({sourceNote:`Open-Meteo fetch failed: ${err.message}. Showing forecast-derived editable values.`}));
      data = Object.assign(data, ext || {});
    }
    if (p.type === 'storm-hazards') data = Object.assign(data, await fetchNwsHazardSignals().catch(err => ({sourceNote:`NWS hazard fetch failed: ${err.message}. Showing forecast-derived hazard index.`})) || {});
    if (p.type === 'clouds-radar') data = Object.assign(data, await fetchNoaaRadarImage().catch(err => ({sourceNote:`NOAA MRMS fetch failed: ${err.message}. Showing editable radar/cloud fallback.`})) || {});
    if (p.type === 'severe-outlook') data = Object.assign(data, await fetchNoaaSpcImage(Number(p.dayOffset || 0)).catch(err => ({sourceNote:`NOAA SPC fetch failed: ${err.message}. Showing editable severe outlook fallback.`})) || {});
    p.data = data;
    p.updated = new Date().toLocaleString('en-US', {month:'short', day:'numeric', hour:'numeric', minute:'2-digit'}).toUpperCase();
    const spec = PRODUCT_TYPES[p.type] || PRODUCT_TYPES.forecast;
    p.title = p.title || spec.title;
    ...
    $('slideSubtitle').value = p.subtitle;
    renderAll();
    setStatus(`${spec.title} product auto-filled for ${selectedProductDate()}. Map/data controls are editable in the Product tab.`);
  } catch (err) {
    console.error(err);
    setStatus(`Product auto-fill failed: ${err.message}. Showing editable fallback product.`);
    p.data = buildProductDataFromCurrent(p.type);
...
/* ----------------------- V14 simplified product maps + alignment repair -----------------------
   Map products no longer fake NOAA maps. User supplies the map image, app supplies editable
   broadcast frame, movable/toggleable key, optional source bar, optional front overlay.
   Rain Chances and Air Quality remain generated chart products. */
function v14MapProductTypes(){ return ['storm-hazards','temperatures','dewpoints','heat-index','clouds-radar','severe-outlook']; }
function isV14MapProduct(type){ return v14MapProductTypes().includes(String(type)); }
function defaultProductState(){
  return {
    type:'forecast', dayOffset:0, title:'', subtitle:'', kicker:'', summary:'', sourceNote:'', data:null, updated:'', manualMapValues:'',
    mapImageUrl:'', mapX:70, mapY:38, mapW:1780, mapH:760, mapImageOpacity:100, showMapFrame:true,
    showMapKey:true, mapKeyX:48, mapKeyY:48, mapKeyScale:100,
    showSourceBar:true, sourceBarX:64, sourceBarY:808, sourceBarWidth:1700,
    showFront:false, frontType:'cold', frontX:390, frontY:260, frontAngle:12, frontLength:760,
    showCityLabels:false, showProductData:false, dataFieldOpacity:0,
    showHazardPanel:true, hazardPanelX:1080, hazardPanelY:95
  };
}
function ensureProduct(){
  state.product = Object.assign(defaultProductState(), state.product || {});
...
function productLayoutClass(type){
  if (type === 'rain-chances' || type === 'air-quality') return 'v14PanelProduct';
  if (isV14MapProduct(type)) return 'v14MapProduct';
  return '';
}
function renderProduct(){
  const p = ensureProduct();
...
function productHtml(p){
  ...
  if (p.type === 'rain-chances') return rainChancesHtml(data, p);
  if (p.type === 'air-quality') return airQualityHtml(data, p);
  if (isV14MapProduct(p.type)) return v14MapProductHtml(data, p);
  return `<div class="productNoData">Choose a product type.</div>`;
}
function v14MapProductHtml(data, p){
  const spec = PRODUCT_TYPES[p.type] || {title:'MAP PRODUCT', subtitle:'RBRTW AREA'};
  const img = String(p.mapImageUrl || '').trim();
  const key = p.showMapKey !== false ? `<div class="v14FloatingKey" style="left:${Number(p.mapKeyX||0)}px;top:${Number(p.mapKeyY||0)}px;transform:scale(${Math.max(.25,Number(p.mapKeyScale||100)/100)});">${v14MapKeyHtml(p.type, data, p)}</div>` : '';
  const source = p.showSourceBar !== false ? `<div class="v14SourceBar" style="left:${Number(p.sourceBarX||0)}px;top:${Number(p.sourceBarY||0)}px;width:${Number(p.sourceBarWidth||1600)}px;">${escapeHtml(productSourceNote(data,p,'Upload or paste your map image. Key, source, and front overlays are editable.'))}</div>` : '';
  const front = p.showFront ? `<div class="v14FrontLine" style="left:${Number(p.frontX||0)}px;top:${Number(p.frontY||0)}px;width:${Number(p.frontLength||0)}px;transform:rotate(${Number(p.frontAngle||0)}deg)"></div>` : '';
  const mapImg = img ? `<img class="v14MapImage" src="${escapeAttr(img)}" alt="uploaded weather map">` : `<div class="v14MapPlaceholder"><div>UPLOAD MAP IMAGE</div><span>${escapeHtml(spec.title)} map frame</span><small>Use Product tab → Map image upload or Map image URL.</small></div>`;
  const frameClass = p.showMapFrame === false ? ' noFrame' : '';
  return `<div class="v14MapStage"><div class="v14MapWindow${frameClass}" style="left:${Number(p.mapX||0)}px;top:${Number(p.mapY||0)}px;width:${Number(p.mapW||1780)}px;height:${Number(p.mapH||760)}px;--mapOpacity:${Math.max(0,Math.min(1,Number(p.mapImageOpacity ?? 100)/100))};">${mapImg}${key}${front}${source}</div></div>`;
}
function v14MapKeyHtml(type, data, p){
  if (type === 'temperatures') return mapLegendForProduct('temp');
  if (type === 'dewpoints') return mapLegendForProduct('dewpoint');
  if (type === 'heat-index') return mapLegendForProduct('heatIndex');
  if (type === 'clouds-radar') return `<div class="radarLegend"><span>LIGHT</span><div></div><span>HEAVY</span></div>`;
  if (type === 'severe-outlook') return `<div class="riskLegend spcTopLegend"><div class="marg">Marginal<br>1</div><div class="slgt">Slight<br>2</div><div class="enh">Enhanced<br>3</div><div class="mod">Moderate<br>4</div><div class="high">High<br>5</div></div>`;
  if (type === 'storm-hazards') {
    const hazards = (data.hazards || defaultHazardsFromDays()).slice(0,4);
    const risk = `<div class="riskLegend stormRiskLegend"><div class="non">NON SEVERE</div><div class="marg">MARGINAL</div><div class="slgt">SLIGHT</div><div class="enh">ENHANCED</div><div class="mod">MODERATE</div><div class="high">HIGH</div></div>`;
    const bars = `<div class="v14HazardBars"><div class="riskScale"><span>LOW</span><span>MODERATE</span><span>HIGH</span></div>${hazards.map(h=>hazardBarHtml(h)).join('')}</div>`;
    return `<div class="v14StormKeyCombo">${risk}${bars}</div>`;
  }
  return '';
}
function rainChancesHtml(data,p){
  const rawDays = (data.rainSeries && data.rainSeries.length ? data.rainSeries : (data.rainDays || shownDays()).map(d => ({date:d.date, pop:numberOnly(d.rainChance) ?? 0}))).slice(0,10);
  const days = rawDays.length ? rawDays : shownDays().map(d=>({date:d.date,pop:numberOnly(d.rainChance)||0}));
  return `<div class="productPanelSlide rainAheadSlide v14RainSlide"><div class="rainChart"><div class="rainBands"><div>LIKELY</div><div>WIDESPREAD</div><div>SCATTERED</div><div>SCATTERED -<br>ISOLATED</div><div>SLIGHT -<br>NONE</div></div><div class="rainBars">${days.map(d => `<div class="rainColumn" style="height:${Math.max(8,Math.min(100,Number(d.pop)||0))}%"><b>${Math.round(Number(d.pop)||0)}%</b><span>${dayName(d.date)}<br>${dateObj(d.date).getDate()}</span></div>`).join('')}</div><div class="rainScale"><div>80-100%</div><div>60-80%</div><div>40-60%</div><div>20-40%</div><div>0-20%</div></div></div>${p.showSourceBar!==false?`<div class="mapProductFooter">${escapeHtml(productSourceNote(data,p,'Source: NWS/Open-Meteo probability of precipitation for selected forecast window.'))}</div>`:''}</div>`;
}
function airQualityHtml(data,p){
  const aqi = Number(data.aqi || 55);
  const cat = data.category || aqiCategory(aqi);
  const point = aqi <= 50 ? 0 : aqi <= 100 ? 1 : aqi <= 150 ? 2 : aqi <= 200 ? 3 : aqi <= 300 ? 4 : 5;
  return `<div class="productPanelSlide airQualitySlide v14AqiSlide"><div class="aqiScale"><div class="aqiGood">Good</div><div class="aqiModerate">Moderate</div><div class="aqiUsG">Unhealthy for<br>Sensitive Groups</div><div class="aqiUnhealthy">Unhealthy</div><div class="aqiVery">Very Unhealthy</div><div class="aqiHaz">Hazardous</div></div><div class="aqiBox"><div class="productKicker">AIR QUALITY INDEX</div><div class="aqiTicker"><span style="grid-column:${Math.max(1,Math.min(6,point+1))}"></span></div><div class="productBigNumber">${escapeHtml(aqi)}</div><div class="productCategory">${escapeHtml(cat)}</div><div class="productSummary">${escapeHtml(productSummaryText(data,p,'Selected-day air quality forecast graphic.'))}</div></div>${p.showSourceBar!==false?`<div class="mapProductFooter">${escapeHtml(productSourceNote(data,p,'Source: Open-Meteo Air Quality API, U.S. AQI forecast variable.'))}</div>`:''}</div>`;
}
async function autoFillProduct(){
  const p = ensureProduct();
  if (p.type === 'forecast') { setStatus('Forecast mode uses Auto Fill NWS for the day cards.'); return; }
  ...
  try {
    let data = buildProductDataFromCurrent(p.type);
    if (p.type === 'rain-chances' || p.type === 'air-quality') {
      const ext = await fetchOpenMeteoProduct(p.type, selectedProductDate()).catch(err => ({sourceNote:`Data fetch failed: ${err.message}. Showing forecast-derived editable values.`}));
      data = Object.assign(data, ext || {});
    }
    if (p.type === 'storm-hazards') {
      const hazards = await fetchNwsHazardSignals().catch(() => null);
      data = Object.assign(data, hazards || {});
    }
    p.data = data;
    p.updated = new Date().toLocaleString('en-US', {month:'short', day:'numeric', hour:'numeric', minute:'2-digit'}).toUpperCase();
    const spec = PRODUCT_TYPES[p.type] || PRODUCT_TYPES.forecast;
    if (!p.title) p.title = spec.title;
    if (!p.subtitle) p.subtitle = spec.subtitle;
    $('slideTitle').value = p.title;
    $('slideSubtitle').value = p.subtitle;
    renderAll();
    setStatus(isV14MapProduct(p.type) ? `${spec.title} template loaded. Upload/paste the map image; key and overlays are editable.` : `${spec.title} product auto-filled for ${selectedProductDate()}.`);
  } catch (err) {
    console.error(err);
    setStatus(`Product auto-fill failed: ${err.message}. Showing editable product.`);
    p.data = buildProductDataFromCurrent(p.type);
    renderAll();
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'Auto Fill Product'; }
...
function handleProductField(e){
  ...
  const key = e.target.dataset.productField;
  let val = e.target.value;
  const numericKeys = ['frontX','frontY','frontAngle','frontLength','mapKeyX','mapKeyY','mapKeyScale','sourceBarX','sourceBarY','sourceBarWidth','hazardPanelX','hazardPanelY','mapImageOpacity','dataFieldOpacity','mapX','mapY','mapW','mapH'];
  if (numericKeys.includes(key)) val = Number(val) || 0;
  p[key] = val;
  if (key === 'title') $('slideTitle').value = val;
  if (key === 'subtitle') $('slideSubtitle').value = val;
...
function renderProductEditor(){
  ...
  const spec = PRODUCT_TYPES[p.type] || PRODUCT_TYPES.forecast;
  if (p.type === 'forecast') { box.innerHTML = 'Forecast Day Builder mode is active. Use the Day tab to edit each forecast card.'; return; }
  const isMap = isV14MapProduct(p.type);
  box.innerHTML = `<div class="productEditorGrid"><button id="refetchProductData" class="primaryBtn fullBtn">${isMap ? 'Load Product Template' : 'Re-fetch Product Data'}</button><label>Product title<input data-product-field="title" value="${escapeAttr(p.title || spec.title)}"></label><label>Product subtitle<input data-product-field="subtitle" value="${escapeAttr(p.subtitle || spec.subtitle)}"></label><label>Summary / lower note<textarea data-product-field="summary">${escapeHtml(p.summary || p.data?.summary || '')}</textarea></label><label>Source note<textarea data-product-field="sourceNote">${escapeHtml(p.sourceNote || p.data?.sourceNote || '')}</textarea></label>${isMap?`<div class="sectionTitle smallTitle">Uploaded Map Image</div><label>Map image URL<input data-product-field="mapImageUrl" placeholder="https://...png/jpg or leave blank and upload" value="${escapeAttr(p.mapImageUrl || '')}"></label><label>Upload map image<input id="productMapUpload" type="file" accept="image/*"></label><div class="grid2"><label>Map X<input data-product-field="mapX" type="number" value="${escapeAttr(p.mapX)}"></label><label>Map Y<input data-product-field="mapY" type="number" value="${escapeAttr(p.mapY)}"></label></div><div class="grid2"><label>Map width<input data-product-field="mapW" type="number" value="${escapeAttr(p.mapW)}"></label><label>Map height<input data-product-field="mapH" type="number" value="${escapeAttr(p.mapH)}"></label></div>${rangeControl('Map image opacity','mapImageOpacity',0,100)}<label class="checkRow"><input type="checkbox" data-product-bool="showMapFrame" ${p.showMapFrame !== false ? 'checked':''}> Show map frame border</label><div class="sectionTitle smallTitle">Map Key / Legend</div><label class="checkRow"><input type="checkbox" data-product-bool="showMapKey" ${p.showMapKey !== false ? 'checked':''}> Show product key</label>${rangeControl('Key X','mapKeyX',0,1700)}${rangeControl('Key Y','mapKeyY',0,820)}${rangeControl('Key scale','mapKeyScale',35,180)}<div class="sectionTitle smallTitle">Pressure / Front Overlay</div><label class="checkRow"><input type="checkbox" data-product-bool="showFront" ${p.showFront === true ? 'checked':''}> Show pressure/front overlay</label>${rangeControl('Front X','frontX',0,1700)}${rangeControl('Front Y','frontY',0,820)}${rangeControl('Front angle','frontAngle',-70,70)}${rangeControl('Front length','frontLength',50,1500)}`:''}<div class="sectionTitle smallTitle">Source Bar</div><label class="checkRow"><input type="checkbox" data-product-bool="showSourceBar" ${p.showSourceBar !== false ? 'checked':''}> Show source / note bar</label>${rangeControl('Source bar X','sourceBarX',0,1700)}${rangeControl('Source bar Y','sourceBarY',0,850)}${rangeControl('Source bar width','sourceBarWidth',300,1800)}<div class="sectionTitle smallTitle">Manual Notes</div><label>Manual values / notes<textarea data-product-field="manualMapValues" placeholder="Optional notes or manual data">${escapeHtml(p.manualMapValues || '')}</textarea></label><button id="refreshProductRender" class="secondaryBtn">Refresh Product Layout</button></div>`;
  $$('[data-product-field]').forEach(el => el.addEventListener('input', handleProductField));
  $$('[data-product-bool]').forEach(el => el.addEventListener('change', handleProductBool));
  $('refreshProductRender')?.addEventListener('click', () => { renderProduct(); setStatus('Product layout refreshed.'); });
  $('refetchProductData')?.addEventListener('click', () => autoFillProduct());
